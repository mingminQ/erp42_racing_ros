 ...
 * @brief   ERP42 Racing platform Qt5 GUI control panel
 * @author  Minkyu Kil
 * @date    2025-06-17
 * @version 1.0
 *
...
#include "erp42_racing_gui/control_panel.hpp"
...
/**
 * @brief Construct the ERP42 Racing GUI control panel widget and initialize the ROS node.
 * @param parent Optional Qt parent widget.
 * @details Calls initialize_node() to set up timers, publishers, service clients, and the UI.
 */
erp42_racing::ControlPanel::ControlPanel(QWidget *parent)
  : QDockWidget(parent), 
    Node("erp42_racing_control_panel")
{
    initialize_node();
}
 ...
 */
erp42_racing::ControlPanel::~ControlPanel()
{
}
 ...
 */
template<>
void erp42_racing::ControlPanel::bind_slider_spin_box<QDoubleSpinBox, double>(
 ...
 */
template<>
void erp42_racing::ControlPanel::bind_slider_spin_box<QSpinBox, int>(
...
/**
 * @brief Build and send an ERP42 Racing ModeCommand via ROS 2 service.
 * @details
 ...
 */
void erp42_racing::ControlPanel::send_mode_command()
{
    ...
    {
        ERP42_RACING_ERROR("ControlPanel::send_mode_command() Service client is not initialized.");
    ...
    {
        ERP42_RACING_ERROR("ControlPanel::send_mode_command() \"/erp42_racing/mode_command\" service is not available.");
        QMessageBox::warning(this, "ERROR", "\"/erp42_racing/mode_command\" service is not available.");
        return;
    }
    ...
    // Mode command
    auto mode_command_request = std::make_shared<erp42_racing_msgs::srv::ModeCommand::Request>();
    ...
    {
        mode_command_request->gear = erp42_racing_msgs::srv::ModeCommand::Request::GEAR_DRIVE;
    }
    else if(control_panel_widget_->neutral_button->isChecked())
    {
        mode_command_request->gear = erp42_racing_msgs::srv::ModeCommand::Request::GEAR_NEUTRAL;
    }
    else if(control_panel_widget_->reverse_button->isChecked())
    {
        mode_command_request->gear = erp42_racing_msgs::srv::ModeCommand::Request::GEAR_REVERSE;
    }

    // Send mode command
    mode_command_client_->async_send_request(mode_command_request,
        [this](rclcpp::Client<erp42_racing_msgs::srv::ModeCommand>::SharedFuture future)
        {
            auto response = future.get();
            if(response && response->success == false)
            {
                ERP42_RACING_ERROR("ControlPanel::send_mode_command() Failed to apply mode.");
 ...
 */
void erp42_racing::ControlPanel::control_command_timer_callback()
{
    if(!control_panel_widget_->activate_control_panel_checkbox->isChecked())
    {
        return;
 ...
 */
void erp42_racing::ControlPanel::initialize_node()
{
    // Timer
    timer_ = this->create_wall_timer(20ms, std::bind(&ControlPanel::control_command_timer_callback, this));

    // Publishers
    control_command_pub_ = this->create_publisher<erp42_racing_msgs::msg::ControlCommand>(
        "/erp42_racing/control_command",
        rclcpp::QoS(rclcpp::KeepLast(1)).reliable().durability_volatile()
    );

    // Service clients
    mode_command_client_ = this->create_client<erp42_racing_msgs::srv::ModeCommand>(
    ...
    bind_slider_spin_box<QDoubleSpinBox, double>(
        control_panel_widget_->speed_slider, control_panel_widget_->speed_spin_box, 0.0, 5.5, 0.1);

    bind_slider_spin_box<QDoubleSpinBox, double>(
        control_panel_widget_->steering_slider, control_panel_widget_->steering_spin_box, -20.0, 20.0, 1.0);
    control_panel_widget_->steering_slider->setValue(20);

    bind_slider_spin_box<QSpinBox, int>(
        control_panel_widget_->brake_slider, control_panel_widget_->brake_spin_box, 0, 100, 10);
}