 ...
 * @brief   ERP42 Racing platform Qt5 feedback monitor
 * @author  Minkyu Kil
 * @date    2025-06-17
 * @version 1.0
 *
...
#include "erp42_racing_gui/feedback_monitor.hpp"
#include "erp42_racing_util/log.hpp"
 ...
 */
erp42_racing::FeedbackMonitor::FeedbackMonitor(QWidget *parent)
  : QDockWidget(parent),
    Node("erp42_racing_feedback_monitor")
{
    initialize_node();
}
 ...
 */
erp42_racing::FeedbackMonitor::~FeedbackMonitor()
{
}
 ...
 * @param[in] msg Shared pointer to @c erp42_racing_msgs::msg::Feedback .
 */
void erp42_racing::FeedbackMonitor::feedback_callback(const erp42_racing_msgs::msg::Feedback::SharedPtr msg)
{
    // Control mode
 ...
 */
bool erp42_racing::FeedbackMonitor::select_target_node(std::string &node_name)
{
 ...
 */
void erp42_racing::FeedbackMonitor::read_vehicle_parameters()
{
    // Select target node (erp42_racing_serial_bridge or erp42_racing_gazebo_bridge)
    std::string node_name;
    if(!select_target_node(node_name))
    {
        ERP42_RACING_ERROR("FeedbackMonitor::read_vehicle_parameters Failed to find target node (erp42_racing_serial_bridge or erp42_racing_gazebo_bridge).");
        return;
    }
    ...
    {
        ERP42_RACING_ERROR("FeedbackMonitor::read_vehicle_parameters Parameter service not available.");
        return;
    }
 ...
 * @details
 * - Subscribes to @c /erp42_racing/feedback with QoS: KeepLast(1), reliable, volatile.
 * - Creates a synchronous parameter client for node @c erp42_racing_serial_bridge .
 * - Sets up the Qt UI and fetches initial parameters to display.
 */
void erp42_racing::FeedbackMonitor::initialize_node()
{
    // Subscibers
    feedback_sub_ = this->create_subscription<erp42_racing_msgs::msg::Feedback>(
