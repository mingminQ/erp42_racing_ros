 ...
 * @brief   Manages serial port communication for ERP42 Racing platform
 * @author  Minkyu Kil
 * @date    2025-06-17
 * @version 1.0
 *
...
#include "erp42_racing_serial/serial_port.hpp"
 ...
 */
erp42_racing_serial::SerialPort::SerialPort(const std::string &port_path, const int &baud_rate)
 ...
 */
void erp42_racing_serial::SerialPort::open_port()
{
    if(port_path_.empty())
    {
        file_descriptor_ = -1;
        throw erp42_racing_util::Exception("SerialPort::open_port() Port path is empty.");
    }
    ...
    {
        file_descriptor_ = -1;
        throw erp42_racing_util::Exception("SerialPort::open_port() File descriptor opening error.");
    }

    this->initialize_port();
    ERP42_RACING_INFO("SerialPort::open_port() Serial port %s has been opened successfully.",
        port_path_.c_str());
}
 ...
 */
void erp42_racing_serial::SerialPort::close_port()
{
    if (file_descriptor_ < 0) 
    {
        file_descriptor_ = -1;
        throw erp42_racing_util::Exception("SerialPort::close_port() Invalid file descriptor.");
    }

    if (close(file_descriptor_) != 0) 
    {
        file_descriptor_ = -1;
        throw erp42_racing_util::Exception("SerialPort::close_port() Failed to close file descriptor.");
    }

    file_descriptor_ = -1;
    ERP42_RACING_INFO("SerialPort::close_port() Serial port %s has been closed successfully.",
        port_path_.c_str());
}
 ...
 */
bool erp42_racing_serial::SerialPort::receive_packet(
    unsigned char *rx_packet, const unsigned int &expected_packet_size
) const
{
    if(file_descriptor_ < 0)
    {
        ERP42_RACING_ERROR("SerialPort::receive_packet() Port is not opened.");
        return false;
    }
 ...
 */
bool erp42_racing_serial::SerialPort::transmit_packet(
    const unsigned char *tx_packet, const unsigned int &expected_packet_size
) const
{
    if(file_descriptor_ < 0)
    {
        ERP42_RACING_ERROR("SerialPort::transmit_packet() Port is not opened.");
        return false;
    }
 ...
 */
void erp42_racing_serial::SerialPort::initialize_port()
{
    struct termios tty;
    ...
    {
        file_descriptor_ = -1;
        throw erp42_racing_util::Exception("SerialPort::initialize_port() tcsetattr failed.");
    }
}