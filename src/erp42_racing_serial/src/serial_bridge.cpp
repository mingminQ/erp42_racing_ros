 ...
 * @brief   ERP42 Racing platform serial bridge
 * @author  Minkyu Kil
 * @date    2025-06-17
 * @version 1.0
 *
...
#include "erp42_racing_serial/serial_bridge.hpp"
#include "erp42_racing_serial/serial_packet.hpp"

#include "erp42_racing_util/exception.hpp"
 ...
 */
erp42_racing_serial::SerialBridge::SerialBridge(const rclcpp::NodeOptions &options)
...
erp42_racing_serial::SerialBridge::~SerialBridge()
{
    ...
    if(!serial_port_)
    {
        ERP42_RACING_ERROR("SerialBridge::~SerialBridge() Serial port %s is deallocated already.", 
            port_path_.c_str());
    }
    else
    {
 ...
 */
bool erp42_racing_serial::SerialBridge::receive_feedback()
{
    ...
    {
        ERP42_RACING_ERROR("SerialBridge::receive_feedback() Packet protocol is ruined.");
        return false;
    }
    ...
    // Battery (V)
    int battery_raw = 0;
    battery_raw |= static_cast<uint16_t>((rx_packet_[RX::BATTERY_0])       & 0xff  );
    battery_raw |= static_cast<uint16_t>((rx_packet_[RX::BATTERY_1] << 8)  & 0xff00);
    feedback_msg.battery = static_cast<double>(battery_raw) * BYTE2BAT;
 ...
 */
bool erp42_racing_serial::SerialBridge::transmit_command()
{
 ...
 */
void erp42_racing_serial::SerialBridge::timer_callback()
{
    transmit_command();
    receive_feedback();
    heartbeat_++;
...
void erp42_racing_serial::SerialBridge::initialize_node()
{
    ...
    if(!serial_port_)
    {
        throw erp42_racing_util::Exception("SerialBridge::initialize_node() Serial port allocation failed.");
    }
    else
    {
...
void erp42_racing_serial::SerialBridge::declare_parameters()
{
    // Serial port
...
RCLCPP_COMPONENTS_REGISTER_NODE(erp42_racing_serial::SerialBridge)
