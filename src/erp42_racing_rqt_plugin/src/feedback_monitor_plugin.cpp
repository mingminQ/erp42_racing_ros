 ...
 * @brief   ERP42 Racing platform RQT feedback monitor GUI plugin
 * @author  Minkyu Kil
 * @date    2025-06-17
 * @version 1.0
 *
...
#include "erp42_racing_rqt_plugin/feedback_monitor_plugin.hpp"
 ...
 */
erp42_racing::rqt_plugin::FeedbackMonitorPlugin::FeedbackMonitorPlugin()
 ...
 * @details This method creates a ROS 2 node named "erp42_racing_feedback_monitor",
 * sets up the UI components, subscribes to the "/erp42_racing/feedback" topic,
 ...
 */
void erp42_racing::rqt_plugin::FeedbackMonitorPlugin::initPlugin(qt_gui_cpp::PluginContext &context)
{
    // Initialize ROS2 node
    node_ = std::make_shared<rclcpp::Node>("erp42_racing_feedback_monitor");
    ...
    // Subscribers
    feedback_sub_ = node_->create_subscription<erp42_racing_msgs::msg::Feedback>(
 ...
 */
void erp42_racing::rqt_plugin::FeedbackMonitorPlugin::shutdownPlugin()
{
 ...
 */
void erp42_racing::rqt_plugin::FeedbackMonitorPlugin::saveSettings(
 ...
 * @param instance_settings Instance-specific settings.
 */
void erp42_racing::rqt_plugin::FeedbackMonitorPlugin::restoreSettings(
...
/**
 * @brief Callback for the ERP42 Racing feedback topic.
 ...
 * @param msg Shared pointer to @c erp42_racing_msgs::msg::Feedback .
 */
void erp42_racing::rqt_plugin::FeedbackMonitorPlugin::feedback_callback(
    const erp42_racing_msgs::msg::Feedback::SharedPtr msg
)
{
    // Control mode
    ...
    const QString gear_text =
        (gear == erp42_racing_msgs::msg::Feedback::GEAR_DRIVE)   ? "Drive"  :
        (gear == erp42_racing_msgs::msg::Feedback::GEAR_REVERSE) ? "Reverse": "Neutral";
    ...
    // Battery
    const double battery = msg->battery;
    const QString battery_text = QString::number(battery, 'f', 3) + " V";
    ...
    QMetaObject::invokeMethod(widget_,
        [this, control_mode_text, emergency_stop_text, gear_text, speed_text, 
            steering_text, brake_text, encoder_text, battery_text, heartbeat_text]()
        {
            ...
            feedback_monitor_widget_->battery_text_box->setPlainText(battery_text);
...
/**
 * @brief Selects the target node (either @c erp42_racing_serial_bridge or @c erp42_racing_gazebo_bridge).
 ...
 */
bool erp42_racing::rqt_plugin::FeedbackMonitorPlugin::select_target_node(std::string &node_name)
{
    ...
    while(rclcpp::ok() && ((std::chrono::steady_clock::now() - starting_time) < timeout))
    {
        ...
        const auto serial_bridge_iter = std::find(node_names.begin(), node_names.end(), "/erp42_racing_serial_bridge");
        if(serial_bridge_iter != node_names.end())
        {
            node_name = "/erp42_racing_serial_bridge";
            return true;
        }

        // Gazebo bridge node exists
        const auto gazebo_bridge_iter = std::find(node_names.begin(), node_names.end(), "/erp42_racing_gazebo_bridge");
        if(gazebo_bridge_iter != node_names.end())
        {
            node_name = "/erp42_racing_gazebo_bridge";
            return true;
        }
 ...
 * @details Selects the target node (either @c erp42_racing_serial_bridge or @c erp42_racing_gazebo_bridge )
 * and reads its parameters. If the service is not available within @c 1000ms, the method
 * returns without updating.
 */
void erp42_racing::rqt_plugin::FeedbackMonitorPlugin::read_vehicle_parameters()
{
    // Select target node (erp42_racing_serial_bridge or erp42_racing_gazebo_bridge)
    ...
    if(!select_target_node(node_name))
    {
        QMessageBox::warning(
            widget_, 
            "ERROR", 
            "FeedbackMonitorPlugin::read_vehicle_parameters Failed to find target node (erp42_racing_serial_bridge or erp42_racing_gazebo_bridge)."
        );
        return;
    }
    ...
    // Read erp42_racing_serial_bridge parameters
    if(node_name == "/erp42_racing_serial_bridge")
    {
    ...
    // Read erp42_racing_gazebo_bridge parameters
    else if(node_name == "/erp42_racing_gazebo_bridge")
    {
        double max_speed = parameter_client_->get_parameter<double>("max_speed_mps");
        feedback_monitor_widget_->max_speed_text_box->setPlainText(QString::number(max_speed, 'f', 3) + " m/s");
...
PLUGINLIB_EXPORT_CLASS(erp42_racing::rqt_plugin::FeedbackMonitorPlugin, rqt_gui_cpp::Plugin)