# erp42_racing_bringup.launch.py
...
def generate_launch_description():
    ...
    erp42_racing_bringup_parameter_file = DeclareLaunchArgument('erp42_racing_bringup_parameter_file', 
        default_value=PathJoinSubstitution([
            FindPackageShare('erp42_racing_bringup'), 'config', 'erp42_racing_bringup.param.yaml'
        ])
    )
    ...
    vehicle_description_file = DeclareLaunchArgument('vehicle_description_file', 
        default_value = PathJoinSubstitution([
            FindPackageShare('erp42_racing_description'), 'urdf', 'erp42_racing.xacro'
        ])
    )
    ...
    rviz_config_file = DeclareLaunchArgument('rviz_config_file', 
        default_value = PathJoinSubstitution([
            FindPackageShare('erp42_racing_description'), 'rviz', 'vehicle_description.rviz'
        ])
    )
    ...
    erp42_racing_serial_bridge = Node(
        ...
        output     = 'screen',
        parameters = [{LaunchConfiguration('erp42_racing_bringup_parameter_file')}]
    )

    # Control panel GUI
    erp42_racing_control_panel = Node(
        package    = 'erp42_racing_gui', 
    ...
    erp42_racing_feedback_monitor = Node(
        package    = 'erp42_racing_gui', 
    ...
    return LaunchDescription([
        ...
        erp42_racing_bringup_parameter_file,
        view_control_panel,
