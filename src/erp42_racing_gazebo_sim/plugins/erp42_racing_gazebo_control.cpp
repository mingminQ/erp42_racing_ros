 ...
 * 
 * @file    erp42_racing_gazebo_control.cpp
 * @brief   ERP42 Racing gazebo-sim control plugin
 * @author  Minkyu Kil
 * @date    2026-03-18
 * @version 1.0
 *
...
#include "erp42_racing_gazebo_sim/erp42_racing_gazebo_control.hpp"
#include "erp42_racing_util/erp42_racing_parameters.hpp"
#include "erp42_racing_msgs/msg/control_command.hpp"
#include "erp42_racing_msgs/msg/feedback.hpp"
#include "erp42_racing_msgs/srv/mode_command.hpp"
...
/** @brief Private implementation for ERP42RacingGazeboControl */
class gz::sim::systems::ERP42RacingGazeboControlPrivate
{
public:
     ...
     */
    void OnControlCommand(const erp42_racing_msgs::msg::ControlCommand::ConstSharedPtr &msg);
    ...
    double steeringLimit{20.0 * M_PI / 180.0};

    // Max vehicle velocity : used to clamp target velocity
    double velocityLimit{5.5};
    ...
    // Target brake ratio : 0 - 100
    std::uint8_t targetBrake{100};
    ...
    std::string childFrame{"erp42_racing"};
    ...
    // Feedback publisher
    rclcpp::Publisher<erp42_racing_msgs::msg::Feedback>::SharedPtr feedbackPublisher;

    // Control command publisher
    rclcpp::Subscription<erp42_racing_msgs::msg::ControlCommand>::SharedPtr controlCommandSubscriber;

    // Mode command service server
    rclcpp::Service<erp42_racing_msgs::srv::ModeCommand>::SharedPtr modeCommandService;

}; // class gz::sim::systems::ERP42RacingGazeboControlPrivate

/**
 * @brief Constructor for ERP42RacingGazeboControl plugin
 * @details Initializes internal state and prepares for configuration
 */
ERP42RacingGazeboControl::ERP42RacingGazeboControl() 
  : dataPtr(std::make_unique<ERP42RacingGazeboControlPrivate>())
{
}
 ...
 * @param[in] _entity The entity associated with the plugin (the ERP42 Racing model)
 ...
 */
void ERP42RacingGazeboControl::Configure(
...
)
{
    ...
    {
        gzerr 
        << "ERP42RacingGazeboControl should be attached to a model entity. " 
    ...
    this->dataPtr->node = std::make_shared<rclcpp::Node>("erp42_racing_gazebo_control", nodeOptions);
    ...
    rclcpp::QoS erp42RacingGazeboControlQoS(rclcpp::KeepLast(1));
    erp42RacingGazeboControlQoS.reliability(rclcpp::ReliabilityPolicy::Reliable);
    erp42RacingGazeboControlQoS.durability(rclcpp::DurabilityPolicy::Volatile);
    ...
    if(this->dataPtr->publishOdometry)
    {
        ...
        this->dataPtr->node->create_publisher<nav_msgs::msg::Odometry>(
            this->dataPtr->odometryTopic, erp42RacingGazeboControlQoS
        );
    }

    // Feedback publisher
    this->dataPtr->feedbackPublisher = 
    this->dataPtr->node->create_publisher<erp42_racing_msgs::msg::Feedback>(
        "/erp42_racing/feedback", erp42RacingGazeboControlQoS
    );

    // Control command subscription
    this->dataPtr->controlCommandSubscriber = 
    this->dataPtr->node->create_subscription<erp42_racing_msgs::msg::ControlCommand>(
        "/erp42_racing/control_command", 
        erp42RacingGazeboControlQoS,
        [this](const erp42_racing_msgs::msg::ControlCommand::ConstSharedPtr &msg)
        {
    ...
    this->dataPtr->node->create_service<erp42_racing_msgs::srv::ModeCommand>(
        "/erp42_racing/mode_command",
        [this](
            const erp42_racing_msgs::srv::ModeCommand::Request::SharedPtr request,
            erp42_racing_msgs::srv::ModeCommand::Response::SharedPtr response
        )
...
void ERP42RacingGazeboControl::PreUpdate(
    const UpdateInfo &_info,
    EntityComponentManager &_ecm
)
{
    GZ_PROFILE("ERP42RacingGazeboControl::PreUpdate");
...
/**
 * @brief Post-update function for ERP42RacingGazeboControl plugin, called after each simulation update
 * @param[in] _info UpdateInfo containing timing and state information
 * @param[in] _ecm The EntityComponentManager for interacting with the simulation
 * @details Handles ROS2 communication, updates odometry and feedback information, 
 * and applies control commands
 */
void ERP42RacingGazeboControl::PostUpdate(
    const UpdateInfo &_info,
    const EntityComponentManager &_ecm
)
{
    GZ_PROFILE("ERP42RacingGazeboControl::PostUpdate");
 ...
 * such as steering angle and speed, and publishes it at a fixed rate defined by feedback_period.
 */
void ERP42RacingGazeboControlPrivate::UpdateFeedback(
    const UpdateInfo &_info, 
    const EntityComponentManager &_ecm
)
{
    GZ_PROFILE("ERP42RacingGazeboControlPrivate::UpdateFeedback");
    ...
    const auto rearLeftWheelPosition = 
    _ecm.Component<components::JointPosition>(this->joints[REAR_LEFT]);

    // Wheel velocity : using actual encoder
    const auto rearLeftWheelVelocity = 
    _ecm.Component<components::JointVelocity>(this->joints[REAR_LEFT]);

    if(
        !leftSteeringPosition  || leftSteeringPosition->Data().empty()  ||
        !rightSteeringPosition || rightSteeringPosition->Data().empty() ||
        !rearLeftWheelPosition || rearLeftWheelPosition->Data().empty() ||
        !rearLeftWheelVelocity || rearLeftWheelVelocity->Data().empty()
    )
    {
        return;
    }
    ...
    erp42_racing_msgs::msg::Feedback feedback;
    ...
    const double rearLeftFactor = 
        1.0 - (this->rearWheelTread * std::tan(feedback.steering)) / (2.0 * this->wheelBase);

    const double rearLeftWheelSpeed =
        rearLeftWheelVelocity->Data()[0] * this->rearWheelRadius;

    feedback.speed = 
        (std::abs(rearLeftFactor) > 0.01) ? 
        (rearLeftWheelSpeed / rearLeftFactor) : 
        rearLeftWheelSpeed;
    ...
    const double encoderAngle = rearLeftWheelPosition->Data()[0];
    ...
    if((2.0 * M_PI / erp42_racing_util::ENCODER_CPR) < std::abs(deltaEncoderAngle))
    {
        // Compute encoder count
        this->encoderCount += static_cast<int>(
            deltaEncoderAngle / (2.0 * M_PI) * erp42_racing_util::ENCODER_CPR
        );
 ...
 */
void ERP42RacingGazeboControlPrivate::UpdateControl(
    const UpdateInfo &_info, 
    const EntityComponentManager &_ecm
)
{
    GZ_PROFILE("ERP42RacingGazeboControlPrivate::UpdateControl");
    ...
    double velocity, steering, brakeRatio;
    {
        ...
        brakeRatio = static_cast<double>(this->targetBrake) / 100.0;
 ...
 * Also broadcasts the corresponding transform for the odometry frame.
 */
void ERP42RacingGazeboControlPrivate::UpdateOdometry(
    const UpdateInfo &_info,
    const EntityComponentManager &_ecm
)
{
    GZ_PROFILE("ERP42RacingGazeboControlPrivate::UpdateOdometry");
 ...
 * and updates the vehicle's joint velocities accordingly.
 */
void ERP42RacingGazeboControlPrivate::OnControlCommand(
    const erp42_racing_msgs::msg::ControlCommand::ConstSharedPtr &msg
)
{
    // Lock the mutex to protect against data
    std::lock_guard<std::mutex> lock(this->mutex);
    ...
    // Brake command is clamped between 0 and 100, where 100 represents full braking force.
    this->targetBrake = std::clamp<std::uint8_t>(msg->brake, 0, 100);
}
 ...
 */
void ERP42RacingGazeboControlPrivate::OnModeCommand(
    const erp42_racing_msgs::srv::ModeCommand::Request::SharedPtr request,
    erp42_racing_msgs::srv::ModeCommand::Response::SharedPtr response
)
{
...
GZ_ADD_PLUGIN(
    ERP42RacingGazeboControl,
    System,
    ERP42RacingGazeboControl::ISystemConfigure,
    ERP42RacingGazeboControl::ISystemPreUpdate,
    ERP42RacingGazeboControl::ISystemPostUpdate
)

GZ_ADD_PLUGIN_ALIAS(
    ERP42RacingGazeboControl,
    "gz::sim::systems::ERP42RacingGazeboControl"
)