...
#include "erp42_racing_gazebo_sim/erp42_racing_control.hpp"
...
/** @brief Private implementation for ERP42RacingControl */
class gz::sim::systems::ERP42RacingControlPrivate
{
public:
    ...
    // Vehicle geometry
    double wheel_base         {1.212};
    double kingpin_width      {0.970};
    double front_wheel_tread  {0.970};
    double rear_wheel_tread   {0.938};
    double front_wheel_radius {0.280};
    double rear_wheel_radius  {0.290};

    // Vehicle dynamics limits
    double steer_p_gain       {1.0};
    double steer_limit        {0.34906585039};
    double velocity_limit     {5.5};
    ...
    // Feedback publisher
    rclcpp::Publisher<erp42_racing_msgs::msg::Feedback>::SharedPtr feedback_pub;

    // Control command publisher
    rclcpp::Subscription<erp42_racing_msgs::msg::ControlCommand>::SharedPtr control_command_sub;

    // Mode command service server
    rclcpp::Service<erp42_racing_msgs::srv::ModeCommand>::SharedPtr mode_command_srv;

}; // class gz::sim::systems::ERP42RacingControlPrivate

/**
 * @brief Constructor for ERP42RacingControl plugin
 * @details Initializes internal state and prepares for configuration
 */
ERP42RacingControl::ERP42RacingControl() : dataPtr(std::make_unique<ERP42RacingControlPrivate>())
{
}
 ...
 */
void ERP42RacingControl::Configure(
...
)
{
    ...
    {
        gzerr << 
            "ERP42RacingControl should be attached to a model entity. " << 
...
void ERP42RacingControl::PreUpdate(
    const UpdateInfo &_info,
    EntityComponentManager &_ecm
)
{
    GZ_PROFILE("ERP42RacingControl::PreUpdate");
...
/**
 * @brief Post-update function for ERP42RacingControl plugin, called after each simulation update
 * @param[in] _info UpdateInfo containing timing and state information
 * @param[in] _ecm The EntityComponentManager for interacting with the simulation
 * @details Handles ROS2 communication, updates odometry and feedback information, 
 * and applies control commands
 */
void ERP42RacingControl::PostUpdate(
    const UpdateInfo &_info,
    const EntityComponentManager &_ecm
)
{
    GZ_PROFILE("ERP42RacingControl::PostUpdate");
 ...
 * Also broadcasts the corresponding transform for the odometry frame.
 */
void ERP42RacingControlPrivate::update_odometry(
    const UpdateInfo &_info,
    const EntityComponentManager &_ecm
)
{
    GZ_PROFILE("ERP42RacingControl::update_odometry");
 ...
 * such as steering angle and speed, and publishes it at a fixed rate defined by feedback_period.
 */
void ERP42RacingControlPrivate::update_feedback(
    const UpdateInfo &_info, 
    const EntityComponentManager &_ecm
)
{
    GZ_PROFILE("ERP42RacingControl::update_feedback");
    ...
    const auto rear_left_wheel_position = 
    _ecm.Component<components::JointPosition>(this->joint_entities[REAR_LEFT]);

    // Wheel velocity : using actual encoder
    const auto rear_left_wheel_velocity = 
    _ecm.Component<components::JointVelocity>(this->joint_entities[REAR_LEFT]);
    ...
    if(
        ...
        !rear_left_wheel_position   || rear_left_wheel_position->Data().empty()   ||
        !rear_left_wheel_velocity   || rear_left_wheel_velocity->Data().empty()
    )
    {
        return;
    }
    ...
    const double rl_factor = 
        1.0 - (rear_wheel_tread * std::tan(feedback.steering)) / (2.0 * wheel_base);

    const double rl_wheel_speed =
        rear_left_wheel_velocity->Data()[0] * rear_wheel_radius;

    feedback.speed = (std::abs(rl_factor) > 0.01) ? (rl_wheel_speed / rl_factor) : rl_wheel_speed;

    // Set brake
    feedback.brake = this->target_brake;

    // Update encoder count
    const double encoder_angle = rear_left_wheel_position->Data()[0];
    ...
    if((2.0 * M_PI / erp42_racing_util::ENCODER_CPR) < std::abs(delta_encoder_angle))
    {
        // Compute encoder count
        this->encoder_count += static_cast<int>(
            delta_encoder_angle / (2.0 * M_PI) * erp42_racing_util::ENCODER_CPR
        );
 ...
 * and updates the vehicle's joint velocities accordingly.
 */
void ERP42RacingControlPrivate::control_command_callback(
    const erp42_racing_msgs::msg::ControlCommand::ConstSharedPtr &msg
)
{
    ...
    this->target_brake = std::clamp<std::uint8_t>(msg->brake, 0, 100);
}
 ...
 */
void ERP42RacingControlPrivate::mode_command_callback(
    const erp42_racing_msgs::srv::ModeCommand::Request::SharedPtr request,
    erp42_racing_msgs::srv::ModeCommand::Response::SharedPtr response
)
{
 ...
 */
void ERP42RacingControlPrivate::update_control(
    const UpdateInfo &_info, 
    const EntityComponentManager &_ecm
)
{
    GZ_PROFILE("ERP42RacingControl::update_control");
    ...
    const double brake_scale = static_cast<double>(brake) / 100.0;
...
GZ_ADD_PLUGIN(
    ERP42RacingControl,
    System,
    ERP42RacingControl::ISystemConfigure,
    ERP42RacingControl::ISystemPreUpdate,
    ERP42RacingControl::ISystemPostUpdate
)

GZ_ADD_PLUGIN_ALIAS(
    ERP42RacingControl,
    "gz::sim::systems::ERP42RacingControl"
)