 ...
 * @brief   ERP42 Racing platform Gazebo bridge
 * @author  Minkyu Kil
 * @date    2025-06-17
 * @version 1.0
 *
...
#include "erp42_racing_gazebo/gazebo_bridge.hpp"
#include "erp42_racing_description/vehicle_parameters.hpp"
...
/**
 * @brief Construct the ERP42 Racing Gazebo bridge node with safe defaults.
 * @details
 * - Initializes rclcpp::Node with the name "erp42_racing_gazebo_bridge".
 ...
 */
erp42_racing::GazeboBridge::GazeboBridge()
  : rclcpp::Node("erp42_racing_gazebo_bridge")
{
    declare_parameters();
    initialize_node();
}
 ...
 *  - Compute vehicle speed from the rear-left wheel angular velocity times wheel radius.
 ...
 */
void erp42_racing::GazeboBridge::joint_state_callback(const sensor_msgs::msg::JointState::SharedPtr msg)
{
 ...
 * @param  msg Shared pointer to erp42_racing_msgs::msg::ControlCommand.
 * @details
 ...
 */
void erp42_racing::GazeboBridge::control_command_callback(const erp42_racing_msgs::msg::ControlCommand::SharedPtr msg)
{
    ...
    // Gear
    if(gear_ == erp42_racing_msgs::srv::ModeCommand::Request::GEAR_REVERSE)
    {
        speed_command_ = -speed_command_;
        steering_command_ = -steering_command_;
    } 
    else if(gear_ == erp42_racing_msgs::srv::ModeCommand::Request::GEAR_NEUTRAL) 
    {
        speed_command_ = 0.0;
    }
    ...
    request->ode_joint_config.damping.push_back(static_cast<double>(brake_command_));
 ...
 */
void erp42_racing::GazeboBridge::mode_command_callback(
 ...
 */
void erp42_racing::GazeboBridge::timer_callback()
{
    ...
    // Publish feedback
    erp42_racing_msgs::msg::Feedback feedback_msg;
 ...
 * - Sets up publishers:
 *     /erp42_racing/cmd_vel   : geometry_msgs/Twist command for the simulator/controller
 *     /erp42_racing/feedback  : erp42_racing_msgs/Feedback telemetry mirror
 * - Sets up subscribers:
 *     /erp42_racing/control_command : high-level control input (speed/steer/brake)
 *     /erp42_racing/joint_states    : joint state feedback from Gazebo (for speed/encoder/steer)
 ...
 */
void erp42_racing::GazeboBridge::initialize_node()
{
    // Timer
    timer_ = this->create_wall_timer(20ms, std::bind(&GazeboBridge::timer_callback, this));

    // Publishers
    cmd_vel_pub_ = this->create_publisher<geometry_msgs::msg::Twist>("/erp42_racing/cmd_vel", 10);
    ...
    // Subscribers
    control_command_sub_ = this->create_subscription<erp42_racing_msgs::msg::ControlCommand>(
    ...
    joint_state_sub_ = this->create_subscription<sensor_msgs::msg::JointState>(
        "/erp42_racing/joint_states", 
        rclcpp::QoS(rclcpp::KeepLast(1)).reliable().durability_volatile(),
        std::bind(&GazeboBridge::joint_state_callback, this, std::placeholders::_1)
    );

    // Services
    mode_command_srv_ = this->create_service<erp42_racing_msgs::srv::ModeCommand>(
    ...
    joint_properties_client_ = this->create_client<gazebo_msgs::srv::SetJointProperties>(
        "/erp42_racing/set_joint_properties"
    );
}

/**
 * @brief Declare and read runtime parameters for ERP42 Racing limits and steering offset.
 * @details
