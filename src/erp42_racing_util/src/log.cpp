...
namespace // anonymous namespace for internal linkage
{
    ...
    struct LogManager
    {
        ...
        erp42_racing::log::LogLevel log_level_;

        // Activated handler type
        erp42_racing::log::LogType log_type_;

        // Pointer to active handler
        erp42_racing::log::LogHandler *current_handler_;

        // Terminal log handler instance
        erp42_racing::log::LogHandlerTerminal *terminal_log_handler_;

        //  File handler instance, if enabled
        erp42_racing::log::LogHandlerFile *file_log_handler_;
        ...
        LogManager()
        {
            ...
            log_level_ = erp42_racing::log::LogLevel::INFO;

            // File log handler is not activated at default
            terminal_log_handler_ = new erp42_racing::log::LogHandlerTerminal();
            file_log_handler_     = nullptr;

            // Default log handler is terminal log handler
            log_type_        = erp42_racing::log::LogType::TERMINAL;
...
// "erp42_racing::log::LogHandlerTerminal" source
 ...
 */
void erp42_racing::log::LogHandlerTerminal::print_log(const char *file_name, int line_number, LogLevel log_level, const std::string &msg)
{
    // Linux system only
    // bool isTTY(isatty(fileno(stderr)) != 0);
    bool isTTY = true;

    if(erp42_racing::log::LogLevel::WARNING <= log_level)
    {
        if(isTTY)
        {
    ...
    else // log_level <= erp42_racing::log::LogLevel::INFO
    {
        if(isTTY)
        {
 ...
 */
erp42_racing::log::LogHandlerFile::LogHandlerFile(const char *file_name)
{
...
erp42_racing::log::LogHandlerFile::~LogHandlerFile()
{
    if(file_ != nullptr)
    {
        if(fclose(file_) != 0)
 ...
 * @param msg Formatted log message
 */
void erp42_racing::log::LogHandlerFile::print_log(const char *file_name, int line_number, LogLevel log_level, const std::string &msg)
{
    if(file_ != nullptr)
    {
        if(erp42_racing::log::LogLevel::WARNING <= log_level)
 ...
 */
void erp42_racing::log::set_log_level(LogLevel log_level)
{
    GET_LOG_MANAGER;
 ...
 */
erp42_racing::log::LogLevel erp42_racing::log::get_log_level()
{
    GET_LOG_MANAGER;
 ...
 * @param file_name Path to the log file
 */
void erp42_racing::log::activate_log_file(const std::string &file_name)
{
    GET_LOG_MANAGER;
...
void erp42_racing::log::deactivate_log_file()
{
    GET_LOG_MANAGER;
    if(log_manager->file_log_handler_ == nullptr)
    {
 ...
 */
void erp42_racing::log::set_log_type(LogType log_type)
{
    GET_LOG_MANAGER;
    if(log_type != log_manager->log_type_)
    {
        ...
        else // log_type == erp42_racing::log::TERMINAL_HANDLER
        {
 ...
 */
erp42_racing::log::LogType erp42_racing::log::get_log_type()
{
    GET_LOG_MANAGER;
 ...
 */
void erp42_racing::log::print_log(const char *file_name, int line_number, LogLevel log_level, const char *msg, ...)
{
    GET_LOG_MANAGER;
